...
void set_default_settings()
{
	...
	settings->setDefault("serverlist_url", "https://servers.dnd3.org");
	...
	// see <https://github.com/dnd3-org/dnd3/issues/12792>
	...
	// Pixel art rendering
	// voxel_visual_scale: scales geometry in clip-space so nodes appear smaller/larger
	// on screen without changing collision (1.0 = unchanged, 0.75 = 75% of original size).
	settings->setDefault("voxel_visual_scale", "0.75");
	settings->setDefault("render.voxel_visual_scale", "0.75");
	// Target texel density used by high-detail texture profiles.
	settings->setDefault("render.voxel_texel_density", "128");
	// Tactical Diorama Engine profile: preferred visual-scale key.
	settings->setDefault("render.diorama_visual_scale", "0.55");
	// Pixel-grid snap factor used by second stage post-process.
	settings->setDefault("render.pixel_scale", "2.0");
	settings->setDefault("dnd3_diorama_enabled", "true");
	settings->setDefault("dnd3_diorama_max_slope_step", "1");
	settings->setDefault("dnd3_diorama_smooth_movement", "true");
	settings->setDefault("dnd3_diorama_smooth_max_step", "1.0");
	settings->setDefault("dnd3_diorama_camera_mode", "isometric");
	// pixel_art_cel_shading: quantizes scene lighting into discrete steps for a
	// toon/cel-shaded look that complements pixel art textures.
	settings->setDefault("pixel_art_cel_shading", "true");
	settings->setDefault("pixel_art_cel_shading_steps", "6");
	// pixel_art_outlines: Sobel edge-detection pass in the post-process stage
	// that draws dark outlines on geometry boundaries.
	// Requires enable_post_processing = true.
	settings->setDefault("pixel_art_outlines", "true");
	settings->setDefault("pixel_art_outline_strength", "1.0");
	...
	// ContentDB
	settings->setDefault("contentdb_url", "https://content.dnd3.org");
	...
	settings->setDefault("update_information_url", "https://www.dnd3.org/release_info.json");
#else
