...
public:
	...
	void onGenerate(const std::string &name, ShaderConstants &constants) override
	{
		...
		// --- Pixel art rendering ---
		// Clip-space scale: makes every node appear visually smaller without
		// touching physics.  1.0 = no change.  0.75 = 75% apparent size.
		float vvs = g_settings->exists("render.diorama_visual_scale") ?
			g_settings->getFloat("render.diorama_visual_scale") :
			(g_settings->exists("render.voxel_visual_scale") ?
				g_settings->getFloat("render.voxel_visual_scale") :
				g_settings->getFloat("voxel_visual_scale"));
		constants["VOXEL_VISUAL_SCALE"] = vvs;

		float texel_density = g_settings->exists("render.voxel_texel_density") ?
			g_settings->getFloat("render.voxel_texel_density") : 128.0f;
		constants["VOXEL_TEXEL_DENSITY"] = texel_density;

		float pixel_scale = g_settings->exists("render.pixel_scale") ?
			g_settings->getFloat("render.pixel_scale") : 2.0f;
		constants["PIXEL_SNAP_SCALE"] = std::max(1.0f, pixel_scale);

		// Cel-shading: quantise scene luminance into discrete bands.
		if (g_settings->getBool("pixel_art_cel_shading")) {
			constants["ENABLE_CEL_SHADING"] = 1;
			s32 steps = std::max(2, g_settings->getS32("pixel_art_cel_shading_steps"));
			constants["CEL_SHADING_STEPS"] = steps;
		}

		// Post-process outline (Sobel).  Only active when post-processing is on.
		if (g_settings->getBool("pixel_art_outlines") &&
				g_settings->getBool("enable_post_processing")) {
			constants["ENABLE_PIXEL_ART_OUTLINES"] = 1;
			float ost = g_settings->getFloat("pixel_art_outline_strength");
			constants["PIXEL_ART_OUTLINE_STRENGTH"] = ost;
		}
	}
};
