...
#ifdef ENABLE_CEL_SHADING
	// Quantise lighting into discrete bands for a toon/pixel-art look.
	{
		float lum = dot(col.rgb, vec3(0.2126, 0.7152, 0.0722));
		float q = floor(lum * float(CEL_SHADING_STEPS) + 0.5) / float(CEL_SHADING_STEPS);
		col.rgb *= q / max(lum, 0.001);
	}
#endif
