...
install_linux_deps() {
	...
	if [ -n "$MINETEST_POSTGRESQL_CONNECT_STRING" ]; then
		...
		sudo -u postgres psql <<<"
			CREATE USER dnd3 WITH PASSWORD 'dnd3';
			CREATE DATABASE dnd3;
			\c dnd3
			GRANT ALL ON SCHEMA public TO dnd3;
		"
	fi
}
...
install_macos_precompiled_deps() {
	...
	cd /Users/Shared
	wget -O macos${osver}_${arch}_deps.tar.gz https://github.com/dnd3-org/luanti_macos_deps/releases/download/latest/macos${osver}_${arch}_deps.tar.gz
