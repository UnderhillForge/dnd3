-- dnd3.lua
-- Packet dissector for the UDP-based dnd3 protocol
...
-- global reference to 'dnd3.peer' field (set later)
...
do
	local f_langlen, f_lang =
		minetest_field_helper("uint16", "dnd3.client.init2_language", "Language Code")
...
do
	local abbr = "dnd3.client.playerpos_"
...
do
	local f_count = ProtoField.uint8("dnd3.client.gotblocks_count", "Count", base.DEC)
	local f_block = ProtoField.bytes("dnd3.client.gotblocks_block", "Block", base.NONE)
	local f_x = ProtoField.int16("dnd3.client.gotblocks_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("dnd3.client.gotblocks_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("dnd3.client.gotblocks_z", "Block position Z", base.DEC)
...
do
	local f_count = ProtoField.uint8("dnd3.client.deletedblocks_count", "Count", base.DEC)
	local f_block = ProtoField.bytes("dnd3.client.deletedblocks_block", "Block", base.NONE)
	local f_x = ProtoField.int16("dnd3.client.deletedblocks_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("dnd3.client.deletedblocks_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("dnd3.client.deletedblocks_z", "Block position Z", base.DEC)
...
do
	local f_action = ProtoField.string("dnd3.client.inventory_action", "Action")
...
do
	local f_length = ProtoField.uint16("dnd3.client.chat_message_length", "Length", base.DEC)
	local f_message = ProtoField.string("dnd3.client.chat_message", "Message")
...
do
	local f_amount = ProtoField.uint8("dnd3.client.damage_amount", "Amount", base.DEC)
...
do
	local f_item = ProtoField.uint16("dnd3.client.playeritem_item", "Wielded item")
...
do
	local abbr = "dnd3.client.interact_"
	local vs_action = {
		[0] = "Start digging",
		[1] = "Stop digging",
		[2] = "Digging completed",
...
do
	local abbr = "dnd3.client.client_ready_"
...
do
	local abbr = "dnd3.server.auth_accept_"
...
do
	local f_x = ProtoField.int16("dnd3.server.blockdata_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("dnd3.server.blockdata_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("dnd3.server.blockdata_z", "Block position Z", base.DEC)
	local f_data = ProtoField.bytes("dnd3.server.blockdata_block", "Serialized MapBlock")
...
do
	local f_x = ProtoField.int16("dnd3.server.addnode_x", "Position X", base.DEC)
	local f_y = ProtoField.int16("dnd3.server.addnode_y", "Position Y", base.DEC)
	local f_z = ProtoField.int16("dnd3.server.addnode_z", "Position Z", base.DEC)
	local f_data = ProtoField.bytes("dnd3.server.addnode_node", "Serialized MapNode")
...
do
	local f_x = ProtoField.int16("dnd3.server.removenode_x", "Position X", base.DEC)
	local f_y = ProtoField.int16("dnd3.server.removenode_y", "Position Y", base.DEC)
	local f_z = ProtoField.int16("dnd3.server.removenode_z", "Position Z", base.DEC)
...
do
	local f_inventory = ProtoField.string("dnd3.server.inventory", "Inventory")
...
do
	local f_time = ProtoField.uint16("dnd3.server.time_of_day", "Time", base.DEC)
	local f_time_speed = ProtoField.float("dnd3.server.time_speed", "Time Speed", base.DEC)
...
do
	local abbr = "dnd3.server.chat_message_"
	local vs_type = {
		[0] = "Raw",
		[1] = "Normal",
		[2] = "Announce",
...
do
	local f_removed_count = ProtoField.uint16(
		"dnd3.server.active_object_remove_add_removed_count",
		"Count of removed objects", base.DEC)
	local f_removed = ProtoField.bytes(
		"dnd3.server.active_object_remove_add_removed",
		"Removed object")
	local f_removed_id = ProtoField.uint16(
		"dnd3.server.active_object_remove_add_removed_id",
		"ID", base.DEC)

	local f_added_count = ProtoField.uint16(
		"dnd3.server.active_object_remove_add_added_count",
		"Count of added objects", base.DEC)
	local f_added = ProtoField.bytes(
		"dnd3.server.active_object_remove_add_added",
		"Added object")
	local f_added_id = ProtoField.uint16(
		"dnd3.server.active_object_remove_add_added_id",
		"ID", base.DEC)
	local f_added_type = ProtoField.uint8(
		"dnd3.server.active_object_remove_add_added_type",
		"Type", base.DEC)
	local f_added_init_length = ProtoField.uint32(
		"dnd3.server.active_object_remove_add_added_init_length",
		"Initialization data length", base.DEC)
	local f_added_init_data = ProtoField.bytes(
		"dnd3.server.active_object_remove_add_added_init_data",
		"Initialization data")
...
do
	local f_object_count = ProtoField.uint16(
		"dnd3.server.active_object_messages_object_count",
		"Count of objects", base.DEC)
	local f_object = ProtoField.bytes(
		"dnd3.server.active_object_messages_object",
		"Object")
	local f_object_id = ProtoField.uint16(
		"dnd3.server.active_object_messages_id",
		"ID", base.DEC)
	local f_message_length = ProtoField.uint16(
		"dnd3.server.active_object_messages_message_length",
		"Message length", base.DEC)
	local f_message = ProtoField.bytes(
		"dnd3.server.active_object_messages_message",
		"Message")
...
do
	local f_hp = ProtoField.uint16("dnd3.server.hp", "Health points", base.DEC)
...
do
	local abbr = "dnd3.server.move_player_"
...
do
	local f_reasonlen, f_reason = minetest_field_helper("uint16",
		"dnd3.server.access_denied_reason", "Reason")
...
do
	local abbr = "dnd3.server.hudchange_"
	local vs_stat = {
		[0] = "pos",
		[1] = "name",
		[2] = "scale",
...
do
	local abbr = "dnd3.server.update_player_list_"
	local vs_type = {
		[0] = "Init",
		[1] = "Add",
		[2] = "Remove",
...
-- dnd3.control dissector

do
	local p_control = Proto("dnd3.control", "dnd3 Control")
	...
	local f_control_type = ProtoField.uint8("dnd3.control.type", "Control Type", base.DEC, vs_control_type)
	local f_control_ack = ProtoField.uint16("dnd3.control.ack", "ACK sequence number", base.DEC)
	local f_control_peerid = ProtoField.uint8("dnd3.control.peerid", "New peer ID", base.DEC)
...
-- dnd3.client dissector
-- dnd3.server dissector

-- Defines the dnd3.client or dnd3.server Proto. These two protocols
...
function minetest_define_client_or_server_proto(is_client)
	...
	local proto = Proto(proto_name, "dnd3 " .. this_peer .. " to " .. other_peer)
	...
	-- dnd3.client or dnd3.server dissector function
...
minetest_define_client_or_server_proto(true)  -- dnd3.client
minetest_define_client_or_server_proto(false) -- dnd3.server

-- dnd3.split dissector

do
	local p_split = Proto("dnd3.split", "dnd3 Split Message")

	local f_split_seq = ProtoField.uint16("dnd3.split.seq", "Sequence number", base.DEC)
	local f_split_chunkcount = ProtoField.uint16("dnd3.split.chunkcount", "Chunk count", base.DEC)
	local f_split_chunknum = ProtoField.uint16("dnd3.split.chunknum", "Chunk number", base.DEC)
	local f_split_data = ProtoField.bytes("dnd3.split.data", "Split message data")
	...
	function p_split.dissector(buffer, pinfo, tree)
		...
		if buffer(4,2):uint() == 0 then
			...
			if peer_id.value and peer_id.value == 1 then
				Dissector.get("dnd3.server"):call(buffer(6):tvb(), pinfo, tree)
			elseif peer_id.value then
				Dissector.get("dnd3.client"):call(buffer(6):tvb(), pinfo, tree)
			end
		end
	end
end
...
do
	local p_minetest = Proto("dnd3", "dnd3")
	...
	local f_id = ProtoField.uint32("dnd3.id", "Protocol ID", base.HEX, vs_id)
	local f_peer = ProtoField.uint16("dnd3.peer", "Peer", base.DEC, vs_peer)
	local f_channel = ProtoField.uint8("dnd3.channel", "Channel", base.DEC)
	local f_type = ProtoField.uint8("dnd3.type", "Type", base.DEC, vs_type)
	local f_seq = ProtoField.uint16("dnd3.seq", "Sequence number", base.DEC)
	local f_subtype = ProtoField.uint8("dnd3.subtype", "Subtype", base.DEC, vs_type)

	p_minetest.fields = { f_id, f_peer, f_channel, f_type, f_seq, f_subtype }

	local data_dissector = Dissector.get("data")
	local control_dissector = Dissector.get("dnd3.control")
	local client_dissector = Dissector.get("dnd3.client")
	local server_dissector = Dissector.get("dnd3.server")
	local split_dissector = Dissector.get("dnd3.split")

	function p_minetest.dissector(buffer, pinfo, tree)

		-- Defer if payload doesn't have dnd3's magic number
		...
		pinfo.cols.protocol = "dnd3"
		pinfo.cols.info = "dnd3"
		...
		t:set_text("dnd3, Peer: " .. peer_id .. ", Channel: " .. buffer(6,1):uint())
...
minetest_peer_field = Field.new("dnd3.peer")
