...
* `mods/modpack/moddir` - Relative path to the mod
    * Must be one of the following:
        * `mods/`: mods in the user path's mods folder (ex. `/home/user/.dnd3/mods`)
        * `share/`: mods in the share's mods folder (ex. `/usr/share/dnd3/mods`)
...
`PostgreSQL` backend specific settings:
    ...
    pgsql_connection = host=127.0.0.1 port=5432 user=mt_user password=mt_password dbname=dnd3
...
dnd3 maps consist of `MapBlock`s, each holds 16x16x16 nodes.
...
Initially, dnd3 stored maps in a format called the "sectors" format.
...
Eventually dnd3 outgrew this directory structure, as filesystems were
...
* Indicates if the light is correct at the sides of a map block.
  ...
  If these flags are false, dnd3 will automatically recompute light
...
* Example: if the block at `(0, 0, 0)` has `lighting_complete = 0b1111111111111110`,
  dnd3 will correct lighting in the day light bank when the block at
