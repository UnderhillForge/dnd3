...
```bash
docker buildx build --target dev -t dnd3-dev:0 .
```
The `--target dev` option instructs docker to build the intermediate development image, and the `-t dnd3-dev:0` option tags the image so we can refer to it by that name.
...
```bash
docker run -it \
  --mount type=bind,source=/home/bob/dnd3,target=/dnd3 \
  dnd3-dev:0
```
