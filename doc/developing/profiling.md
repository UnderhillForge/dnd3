...
```bash
perf record -z --call-graph dwarf -- ./bin/dnd3
```
...
frames that took longer than usual. dnd3 already contains annotations for
its frames.
...
Tracy consists of a client (dnd3) and a server (the gui).

Install the server, e.g. using your package manager.

### Building

Build dnd3 with `-DDBUILD_WITH_TRACY=1`, this will fetch Tracy for building
...
Start the Tracy server and dnd3. You should see dnd3 in the menu.
...
If built with Tracy, dnd3 loads its API in the global `tracy` table.
