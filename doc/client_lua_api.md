dnd3 Lua Client Modding API Reference 5.16.0
==============================================
...
Content and functionality can be added to dnd3 by using Lua
scripting in run-time loaded mods.

A mod is a self-contained bunch of scripts, textures and other related
things that is loaded by and interfaces with dnd3.
...
1) `$path_user/dnd3.conf` contains the setting `enable_client_modding = true`
...
* `RUN_IN_PLACE=0`: (Linux release)
    * `$path_share`:
        * Linux: `/usr/share/dnd3`
        * Windows: `<install directory>/dnd3-0.4.x`
    * `$path_user`:
        * Linux: `$HOME/.dnd3`
        * Windows: `C:/users/<user>/AppData/dnd3` (maybe)
...
* `dnd3/clientmods/` (User-installed mods)

On an installed version on Linux:

* `/usr/share/dnd3/clientmods/`
* `$HOME/.dnd3/clientmods/` (User-installed mods)
...
* `core.settings`: Settings object containing all of the settings from the
  main config file (`dnd3.conf`). Check lua_api.md for class reference.
...
```lua
{
    address = "dnd3.example.org",   -- The domain name/IP address of a remote server or "" for a local server.
