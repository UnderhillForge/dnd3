...
CLion try to determine a base configuration, but dnd3 define it's own presets for easier setup. So you need to
...
On the main toolbar at the top right, you have a dropdown for selecting the CMake profile to use for the build. You have another dropdown next to it to select the build target; by default the `dnd3` executable will be selected, but you may also have to use `IrrlichtMt` for building just the library .
...
[Vcpkg](https://vcpkg.io) is the recommended way of installing dnd3 dependencies.
...
dnd3 dependencies in your vcpkg installation, you can do it right from CLion's Vcpkg tool window.
