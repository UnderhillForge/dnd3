dnd3 Lua Modding API Reference
================================

**WARNING**: if you're looking for the `dnd3` namespace (e.g. `dnd3.something`),
it's now called `core` due to the renaming of dnd3 (formerly dnd3).
`dnd3` will keep existing as an alias, so that old code won't break.

Note that `core` has already existed since version 0.4.10, so you can use it
safely without breaking backwards compatibility.

* More information at <http://www.dnd3.org/>
* Additional documentation: <https://docs.dnd3.org/>
* (Unofficial) dnd3 Modding Book by rubenwardy: <https://rubenwardy.com/minetest_modding_book/>
* Modding tools: <https://github.com/dnd3-org/modtools>

Introduction
------------

Content and functionality can be added to dnd3 using Lua scripting
in run-time loaded mods.

A mod is a self-contained bunch of scripts, textures and other related
things, which is loaded by and interfaces with dnd3.
...
dnd3 keeps and looks for files mostly in two paths. `path_share` or `path_user`.
...
`/usr/share/dnd3` while the user path resides in `.dnd3` in the home directory.
...
but for some functions and data structures, dnd3 will only accept integers (whole
numbers).
...
In the Lua runtime used by dnd3, numbers are internally represented
...
You can generally assume that on all systems that dnd3 officially
...
* `game.conf`, with the following keys:
    * `title`: Required, a human-readable title to address the game, e.g. `title = dnd3 Game`.
...
* `name`: The modpack name. Allows dnd3 to determine the modpack name even
...
* `name`: The mod name. Allows dnd3 to determine the mod name even if the
...
wants to register. Subsequent execution depends on dnd3 calling the
...
* Alternative means of supplying data
  * Embedded images. You can use `gltfutil.py` from the
    [modding tools](https://github.com/dnd3-org/modtools) to strip or extract embedded images.
...
you should expect that a future version of dnd3 may respect this,
...
but for now this feature is limited to the ability to override dnd3's default fonts via mods.
It is recommended that this only be used by game mods to set a look and feel.

Warning: Currently the dnd3 client does not support reading kerning information
from the OpenType `GPOS` table, but only the older `kern` table. This can cause
modern fonts not to render correctly in dnd3.
...
dnd3 generally uses nearest-neighbor upscaling for textures to preserve the crisp
...
whole tile. Soft texture overlays were added to dnd3 to allow
...
* `number`: An integer containing the (A)RGB value of the color used to draw the
  text. Specify `0xFFFFFF` for white text, `0x80FF0000` for semi-transparent red, and so on.
    * Alpha only works on dnd3 5.15+ clients. Older clients will see the text as opaque.
...
**WARNING**: dnd3 allows you to add elements to every single formspec instance
...
* `core.strip_escapes(str)`
    * Removes all escape sequences, including client-side translations and
      any unknown or future escape sequences that dnd3 might define.
...
dnd3 uses a **left-handed** coordinate system: Y is "up", X is "right", Z is "forward".
...
dnd3 uses [Tait-Bryan angles](https://en.wikipedia.org/wiki/Euler_angles#Tait%E2%80%93Bryan_angles) for rotations,
...
dnd3 provides two vector classes for working with coordinates and mathematical operations:
...
dnd3 stores 3-dimensional spatial vectors in Lua as tables of 3 coordinates,
...
should be used to ensure seamless compatibility between mods and the dnd3 API.
...
dnd3 stores 2-dimensional vectors in Lua as tables of 2 coordinates,
...
in the dnd3 [modtools](https://github.com/dnd3-org/modtools) repository to
...
dnd3 will look for translations in the `mymod` textdomain as there's no
...
For games and modpacks, dnd3 will look for the textdomain in all mods.
...
dnd3 will then look for the textdomain `mygame` in all mods, for example,
...
Usually in dnd3 the input values are either 2D or 3D coordinates in nodes.
...
dnd3 Schematic file (`.mts`) or through raw data supplied through Lua,
...
dnd3 includes a set of built-in privileges that control capabilities
provided by the dnd3 engine and can be used by mods:
...
dnd3 includes the following settings to control behavior of privileges:
...
* `core.get_modpath(modname)`: returns the directory path for a mod,
  e.g. `"/home/user/.dnd3/usermods/modname"`.
...
* `core.get_worldpath()`: returns e.g. `"/home/user/.dnd3/world"`
    * Useful for storing custom data
* `core.get_mod_data_path()`: returns e.g. `"/home/user/.dnd3/mod_data/mymod"`
    * Useful for storing custom data *independently of worlds*.
    * Must be called during mod load time.
    * Can read or write to this directory at any time.
    * It's possible that multiple dnd3 instances are running at the same
...
* `core.get_player_information(player_name)`: Table containing information
  ...
  ```lua
  {
      ...
      -- Use `core.protocol_versions` to map dnd3 versions to protocol versions.
...
* `core.protocol_versions`:
    * Table mapping dnd3 versions to corresponding protocol versions for modder convenience.
    * For example, to check whether a client has at least the feature set
      of dnd3 5.8.0 or newer, you could do:
...
* `core.get_player_window_information(player_name)`:
  ...
  -- real_*_scaling is just OS DPI / 96 but with another level of user configuration.
  {
      ...
      -- Estimated maximum formspec size before dnd3 will start shrinking the
...
* `core.get_version()`: returns a table containing components of the
    ...
    * `project`: Name of the project, eg, "dnd3"
...
* `core.settings`: Settings object containing all of the settings from the
  main config file (`dnd3.conf`). See [`Settings`](#settings).
...
* `core.get_password_hash(name, raw_password)`
    * Convert a name-password pair to a password hash that dnd3 can use.
...
* `core.get_mapgen_params()`
    ...
    * **WARNING**: `seed` is broken. If the returned `seed` is outside the
        ...
        internally stored by dnd3 is in the [u64] range.
* `core.set_mapgen_params(MapgenParams)`
    ...
    * You may only provide `seed` values in the [ulua] range,
      which is only a part of the full [u64] range that dnd3 internally
      uses for seeds
...
* `core.get_mapgen_setting(name)`
    * Gets the *active* mapgen setting (or nil if none exists) in string
      format with the following order of precedence:
        ...
        3) Settings explicitly set in the user config file, dnd3.conf
    ...
    * Note: The seed is returned as a string, converted from the [u64] integer
      that dnd3 internally uses. Do *not* convert this string to a
        ...
        * `slice_prob_list` is an array of tables containing two fields, `ypos`
          and `prob`.
            ...
            * If slice probability list equals `nil`, no slice probabilities
              are applied.
    * Saves schematic in the dnd3 Schematic format to filename.
...
* `core.request_http_api()`:
    ...
    * Only works at init time and must be called from the mod's main scope
      (not from a function).
    * Function only exists if dnd3 server was built with cURL support.
...
* `dnd3`: alias for the `core` namespace
    * Deprecated, but support is not to be dropped soon
...
* `AreaStore(type_name)`
    ...
    * `type_name`: optional, forces the internally used API.
        ...
        * When other values are specified, or SpatialIndex is not available,
          the custom dnd3 functions are used.
...
* `set_physics_override(override_table)`
    ...
    * For games, we recommend for simpler code to first modify the `movement_*`
      settings (e.g. via the game's `dnd3.conf`) to set a global base value
...
* `set_sky(sky_parameters)`
    ...
    * `sky_parameters` is a table with the following optional fields:
        ...
        * `sky_color`: A table used in `"regular"` type only, containing the
          following values (alpha is ignored):
            ...
            * `fog_tint_type`: string, changes which mode the directional fog
                ...
                `"default"` uses the classic dnd3 sun and moon tinting.
        ...
        * `auto_dim_skybox`: boolean, whether to dim skybox brightness if
          ...
          Requires a dnd3 5.16.0+ client and server.
          (default: `true`)
...
An interface to read config files in the format of `dnd3.conf`.

`core.settings` is a `Settings` instance that can be used to access the
main config file (`dnd3.conf`). Instances for other config files can be
...
* `replacements`: (optional) Allows you to replace input items with some other items
    ...
    * Provided as a list of item pairs of the form `{ old_item, new_item }` where
      ...
      it will become
    * When the output is crafted, dnd3 iterates through the list
...
```lua
{
    ...
    -- working directory of the specified dnd3 schematic file.
...
```lua
{
    ...
    -- Optional, if specified replaces the default dnd3 user agent with
    -- given string.
...
dnd3 can be built with support for the Tracy profiler, which can also be
...
When an error occurs that is not caught, dnd3 calls the function
