# dnd3 Android build
All dnd3 builds, including the Android variant, are based on the same code.
...
## File Path
There are some settings especially useful for Android users. The dnd3-wide
configuration file can usually be found at:

* Before 5.4.2:
    * `/sdcard/dnd3/` or `/storage/emulated/0/` if stored on the device
    * `/storage/emulated/(varying folder name)/` if stored on an SD card
* After 5.4.2:
    * `/sdcard/Android/data/net.dnd3.dnd3/` or `/storage/emulated/0/Android/data/net.dnd3.dnd3/` if stored on the device
    * `/storage/emulated/(varying folder name)/Android/data/net.dnd3.dnd3/` if stored on the SD card
* [Learn more about Android directory](https://docs.dnd3.org/for-players/mobile/)
...
## Requirements
The recommended system requirements for dnd3 are listed below.
...
The new build system dnd3 Android is fully functional and is designed to
